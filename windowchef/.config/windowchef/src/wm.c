...
static void window_stack_reverse(struct client *);
...
static void ipc_window_stack_reverse(uint32_t *);
...
static void
window_stack_reverse(struct client *client)
{
...
register_ipc_handlers(void)
{
	...
	ipc_handlers[IPCWindowStackReverse]    = ipc_window_stack_reverse;
...
static void
ipc_window_stack_reverse(uint32_t *d)
{
	(void)(d);
	if (focused_win == NULL)
		return;

	window_stack_reverse(focused_win);
}
