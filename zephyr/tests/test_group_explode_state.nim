...
suite "independent group explode transactions":
  test "group journals and locks remain independent and history recovery finds group intent":
    let base = getTempDir() / ("zephyr-group-explode-state-" & $getCurrentProcessId())
    let winfo = base / "winfo"
    let a = base / "wme" / "layout" / "explode:group:2"
    let b = base / "wme" / "layout" / "explode:group:4"
    createDir(winfo)
    putEnv("WINFO", winfo)
    putEnv("WME", base / "wme")
    ...
    for path in [base / "wme" / "layout" / ".explode:group:2-operation.txn",
        base / "wme" / "layout" / ".explode:group:2.txn", base / ".restore-all.txn"]:
      var lines = readFile(path).splitLines()
      lines[3] = "0"
      writeFile(path, lines.join("\n"))
    writeFile(base / "wme" / "layout" / ".explode:group:2.lock" / "pid", "0\n")
