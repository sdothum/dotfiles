sirocco(1) -- A client for faouzia(1)
==========================================
...
`sirocco` is the client for faouzia(1). It sends a given command to
faouzia(1) as an X client message. Mutation commands don't print anything on
...
* `window_move` <x> <y>:
	Legacy alias that moves the focused window <x> pixels horizontally and <y> pixels
	vertically.

* `window_move_absolute` <x> <y>:
	Legacy alias that moves the focused window to the position given by <x> and <y>.

* `window_resize` <x> <y>:
	Legacy alias that resizes the focused window by <x> and <y>.

* `window_resize_absolute` <width> <height>:
	Legacy alias that resizes the focused window to <width> and <height>.
...
* `wm quit` <exit_status>:
	Quit faouzia with exit_status <exit_status>.

* `wm config` <key> [<values>...]:
	See [CONFIGURING][].

The legacy `wm_quit` and `wm_config` command names remain accepted with the
...
* `window focused`:
	...
	The query also exits nonzero if faouzia does not respond within two seconds.

Information about the current state of faouzia is available through
X properties of the root window. Example:

```
xprop -root FAOUZIA_ACTIVE_GROUPS
```
...
```
xprop -id 0x02c00009 FAOUZIA_STATUS
```
...
* `enable_resize_hints` <BOOL>:
	If true, `faouzia` will respect window resize hints as defined by ICCCM.
...
* `enable_last_window_focusing` <BOOL>:
	If true, when the currently focused window is unmapped or closed, `faouzia` will focus the
...
* `replay_click_on_focus` <BOOL>:
	If true, when clicking on an unfocused with the intent to focus it, faouzia will also send
...
faouzia(1), sxhkd(1), wmutils(1), pfw(1), lsw(1), chwb2(1), lemonbar(1)
