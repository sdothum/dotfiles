...
Faouzia is written in C with the help of the XCB library for
...
What I want to achieve by writing faouzia is a simple window manager that
...
included in faouzia. Instead, the user is encouraged to write a script that
...
See the bundled `.sxhkdrc` and `.faouziarc` for an example of a basic
configuration. Additionally, you can read the manual pages (`faouzia(1)` and `sirocco(1)`).

The default configuration file is `~/.config/faouzia/faouziarc`. It is usually
...
If you want to use window rules, a feature that faouzia lacks, you can use
...
Faouzia depends on `xcb` to communicate with the X11 server, `xcb-randr` to
...
Building faouzia and installing it
-------------------------------------

Available in the AUR:

* [faouzia](https://aur.archlinux.org/packages/faouzia/)
* [faouzia-git](https://aur.archlinux.org/packages/faouzia-git/)
...
* 100% compatible with window management utilities (e.g. [wmutils](https://github.com/wmutils/)). Faouzia will update its internal state if programs attempt to manage windows. It also applies the correct window decorations when a program focuses a window. This makes it possible to use it as a "backend" with wm utilities.
...
Faouzia allows you to add/remove windows to/from groups, show groups, hide
groups or toggle them.

You can also activate *sticky group mode*. When activated, new windows are
automatically assigned to the currently selected group. Together with the `group_activate_specific` command (see `sirocco(1)`), a workspace-like workflow can be achieved.

### Virtual grids

You can tell faouzia to move and resize a window so it can fit in a cell
...
~~Faouzia doesn't offer any mouse moving/resizing features. You can emulate
...
You can also configure faouzia to require pressing the modifier key to focus
windows.
...
Faouzia doesn't come with a bar/panel on its own. Faouzia ignores
