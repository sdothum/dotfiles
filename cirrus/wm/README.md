...
canonical overlap-stack query and directional focus navigation
...
translate keyboard and pointer events to `sirocco` commands.

Sirocco is a program that sends commands to the window manager through X client
messages. It doesn't print anything on success. The commands and the
parameters are fed as program arguments and sirocco delivers them to the
cirrus.

Cirrus is written in C with the help of the XCB library for
...
What I want to achieve by writing cirrus is a simple window manager that
...
included in cirrus. Instead, the user is encouraged to write a script that
...
See the bundled `.sxhkdrc` and `.cirrusrc` for an example of a basic
configuration. Additionally, you can read the manual pages (`cirrus(1)` and `sirocco(1)`).

The default configuration file is `~/.config/cirrus/cirrusrc`. It is usually
a shell script that calls `sirocco wm config` for configuration. See the
manual page for `sirocco(1)` for details regarding this topic.

If you want to use window rules, a feature that cirrus lacks, you can use
...
Cirrus depends on `xcb` to communicate with the X11 server, `xcb-randr` to
...
Building cirrus and installing it
-------------------------------------

Available in the AUR:

* [cirrus](https://aur.archlinux.org/packages/cirrus/)
* [cirrus-git](https://aur.archlinux.org/packages/cirrus-git/)
...
* Configuration script. Cirrus loads a given script at startup that can be
	used for configuration with sirocco.
* 100% compatible with window management utilities (e.g. [wmutils](https://github.com/wmutils/)). Cirrus will update its internal state if programs attempt to manage windows. It also applies the correct window decorations when a program focuses a window. This makes it possible to use it as a "backend" with wm utilities.
...
Cirrus allows you to add/remove windows to/from groups, show groups, hide
groups or toggle them.

You can also activate *sticky group mode*. When activated, new windows are
automatically assigned to the last activated group.

### Virtual grids

You can tell cirrus to move and resize a window so it can fit in a cell
...
~~Cirrus doesn't offer any mouse moving/resizing features. You can emulate
...
You can also configure cirrus to require pressing the modifier key to focus
windows.
...
Cirrus doesn't come with a bar/panel on its own. Cirrus ignores
