...
create_frame(struct client *client)
{
	...
	trace_restart("create_frame client=0x%08x frame=0x%08x event_mask=0x%x mapped=%d",
			client->window, client->frame, values[3], client->mapped);
...
destroy_frame(struct client *client)
{
	...
	trace_restart("destroy frame=0x%08x client=0x%08x mapped=%d",
			client->frame, client->window, client->mapped);
