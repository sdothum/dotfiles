...
# NOTE: modified for columnar output (vertical alignment of line content):
...
" find %{
	...
	eval -draft -save-regs '"' %{
		...
		eval -no-hooks -buffer * %{
			try %{
				...
				eval -save-regs 'c"' -itersel %{
					reg c "%val{bufname}:%val{cursor_line}:%val{cursor_column}│"  # NOTE: replace column: with visual separator :-)
		...
		edit -scratch *find*
		exec R
		execute-keys "%%|sed 's/^[^:]*:[^:]*:[^│]*│/&䷀/' | column -t -s䷀ -R1 -o'  '<ret>gg"  # NOTE: present "source│ text" result buffer in columnar format
