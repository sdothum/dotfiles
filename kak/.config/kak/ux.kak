...
addm %{ comment c : map global format <c-c>   ': comment-line<ret>'               -docstring 'comment,block  (kak)' }
addm %{ comment c : map global format c       ': comment-block<ret>'              -docstring 'comment,block  (kak)' }
...
nopm %{ line    0 : format - }
addm %{ line    L : map global format l       'x|comment l .<ret>'                -docstring 'leader     ... xxx' }
addm %{ line    l : map global format t       'x|comment t .<ret>'                -docstring 'trailer    xxx ...' }
addm %{ line    r : map global format R       'x|comment r =<ret>'                -docstring 'ruler      ═══'     }
addm %{ line    R : map global format r       'x|comment r --<ret>'               -docstring 'ruler      ━━━'     }
addm %{ line    u : map global format U       'x|comment u =<ret>'                -docstring 'underline  ═══'     }
addm %{ line    U : map global format u       'x|comment u --<ret>'               -docstring 'underline  ━━━'     }
...
addm %{ align 0 : map global format <space> 'x|align '          -docstring 'align  space   nth+1 word'   }
addm %{ align C : map global format <minus> 'x|align --<ret>'   -docstring 'align     --   comment'      }
...
addm %[ align p : map global format {       'x|align \{<ret>'   -docstring 'align      {   block'        ]  # ATTENTION: %[] to escape '{' :)
addm %{ align p : map global format )       'x|align \)<ret>'   -docstring 'align      )   case'         }
addm %{ align s : map global format ';'     'x|align \;\;<ret>' -docstring 'align     ;;   endcase'      }
addm %{ align u : map global format '\'     'x|align \\<ret>'   -docstring 'align      \   continuation' }
addm %{ align u : map global format ','     'x|align \;\\<ret>' -docstring 'align     ;\   continuation' }
...
nopm %{ paste 0 : edit - }
addm %{ paste p : map global edit p '<a-!> xsel --outafter --clipboard<ret>' -docstring 'clipboard  —— after,before,replace' }
addm %{ paste p : map global edit P '! xsel --outafter --clipboard<ret>'     -docstring 'clipboard  —— after,before,replace' }
addm %{ paste p : map global edit R '| xsel --output --clipboard<ret>'       -docstring 'clipboard  —— after,before,replace' }
...
addm %{ file w : map global buffer W   ': sync<ret>: quit!'               -docstring 'write   —— save,and quit!' }
